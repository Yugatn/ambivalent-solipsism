...
function renderProgramGroups(m){
 ...
 el.querySelectorAll("[data-event-id]").forEach(b=>b.addEventListener("click",e=>{e.stopPropagation();const id=b.dataset.eventId;document.querySelector('[data-i="'+data.events.findIndex(x=>x.id===id)+'"][data-k="start"]')?.scrollIntoView({behavior:"smooth",block:"center"})}));
}
